...
const testDataPath = path.resolve(__dirname, '../__test-data__');

/**
 * Возвращает массив с прямыми потомками корневого узла, у которых тип равен 'rule'
 *
...
const checkSelectorAccordance = (selector, filePath) => {
	...
	const fileNameWithoutExt = path.basename(filePath).replace(/\.post\.css$/, '');

	// Проверяем, что название файла соответствует селектору в файле
	if (rawSelector !== fileNameWithoutExt) {
		throw new Error(`css selector "${selector}" does not match with filename`);
	}
...
const check = (cssDir) => {
	...
	cssFilesPaths.forEach((filePath) => {
		...
		try {
			...
			throw new Error(`error in file ${filePath}, details: ${err.message}`);
		}
	});
};
